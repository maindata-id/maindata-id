...
async def test_start_session(client_with_db):
    ...
    response = client_with_db.post("/start-session")
    ...
    response_with_title = client_with_db.post("/start-session", json={"title": "My Test Session"})
...
async def test_get_session_not_found(client_with_db):
    ...
    response = client_with_db.get(f"/session/{fake_session_id}")
...
async def test_get_session_with_messages(client_with_db):
    ...
    # Start a session
    start_session_response = client_with_db.post("/start-session")
    ...
    add_message_response1 = client_with_db.post(f"/session/{session_id}", json=message1_data)
    assert add_message_response1.status_code == 200

    message2_data = {"role": "assistant", "content": "Hi there!"}
    add_message_response2 = client_with_db.post(f"/session/{session_id}", json=message2_data)
    assert add_message_response2.status_code == 200

    # Get the session data
    get_session_response = client_with_db.get(f"/session/{session_id}")
...
async def test_add_message_to_session(client_with_db):
    ...
    add_message_response = client_with_db.post(f"/session/{session_id}", json=message_data)
...
async def test_add_message_to_nonexistent_session(client_with_db):
    ...
    response = client_with_db.post(f"/session/{fake_session_id}", json=message_data)
    assert response.status_code == 404
